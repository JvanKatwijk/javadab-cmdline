...
//
	   try {
//	just read in some samples to get a decent value for sLevel.
...
//      corresponding samples in the datapart.
	         ...
	         for (ofdmSymbolCount = 1;
	            ...
	            for (int i = 0; i < t_g; i ++) {
//	since java does not have a decent "complex" defined,
//	we do all complex multiplications written out.
	         ...
	         for (ofdmSymbolCount = 4;
	            ...
	            my_dabBackend. processBlock (ofdmBuffer, ofdmSymbolCount);
	         }
...
//	      System. out. println ("stopping the ofdmProcessor");
	...
	public	void	selectService (String s) {
	   ...
	   if (my_ficHandler. is_audioService (s)) {
	      ...
	      if (!as. defined) {
	         System. out. println ("Service " + s + " not defined");
	         return;
	      }
	   ...
	   if (my_ficHandler. is_packetService (s)) {
	      ...
	      System. out. println ("we weten niet wat " + s + " is");
	}
	...
	boolean	try_to_sync () throws Exception {
	   int		counter		= 0;
	   double	cLevel		= 0.0;
	   int		syncBufferIndex	= 0;
...
//	Here we start looking for the null level, i.e. a dip
	   while (cLevel / 100.0  > 0.5 * my_Reader. get_sLevel ()) {
	      my_Reader. getSamples (s_Sample, 0f);
	      envBuffer [syncBufferIndex] =
	                 (s_Sample [0] < 0 ? -s_Sample [0] : s_Sample [0]) +
	                 (s_Sample [1] < 0 ? -s_Sample [1] : s_Sample [1]);
//	update the current strength level
	      cLevel += envBuffer [syncBufferIndex] -
	                envBuffer [(syncBufferIndex - 99) & syncBufferMask];
	      syncBufferIndex = (syncBufferIndex + 1) & syncBufferMask;
	      counter ++;
	      if (counter > 3 * t_F / 2) {  // hopeless
//	         System. out. println ("no dip start " + cLevel / 100 + " " + my_Reader. get_sLevel ());
	         return false;
	      }
	   }
...
//	         System. out. println ("no end of dip");
	      return false;
	      }
	   }
	   return true;
