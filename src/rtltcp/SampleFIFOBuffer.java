...
public class SampleFIFOBuffer {
	...
	// not very efficient, but very convenient and simple to use without risk of bugs
	...
	public synchronized void storeBytes(byte[] input) {
		...
		int halfLen = (input.length / 2); // only full values (ignore for odd number of bytes the last byte!)
		...
		buffer.add(store);
		this.increaseBufferedFloatCount(store.length); //this.bufferedFloatCount+=store.length;
	...
	private byte[] restManage(byte[] bytes) {
		...
		if (bytes.length % 2 != 0) {
			rest = bytes[bytes.length-1]; // save last position // (shall not be used elsewhere!)
		}
		return bytes;
	}
	
	public synchronized int consumeSamples(float[] destination, int amountSamples) {		
		...
		while(nextPos<amountFloats) {
			...
			// stillNeeded = (amountFloats-nextPos); // not necessary to update here, as it is done BEFORE the loop and in the end of the loop (see below) 
			current = this.buffer.peekFirst(); // head element in the list, containing a number of float values
			
			if (this.buffer.size() == 0) {
				return nextPos / 2;
			}
			
			if (current.length <= (stillNeeded)) { // use full amount of current head element 
				System.arraycopy(current, 0, destination, nextPos, current.length);
				this.reduceBufferedFloatCount(current.length); //this.bufferedFloatCount-=current.length;
				this.buffer.removeFirst(); // first element was used and is fully consumed
				...
				System.arraycopy(current, 0, destination, nextPos, stillNeeded);
				int remaining = (current.length - stillNeeded);
				float[] shortened = new float[remaining];
				System.arraycopy(current, stillNeeded, shortened, 0, remaining);
				this.reduceBufferedFloatCount(stillNeeded); //bufferedFloatCount=-stillNeeded;
				this.buffer.set(0, shortened); // replace head element
				
				nextPos+=stillNeeded; // update nextPos  ---> should be nextPos == amountFloats
				stillNeeded = (amountFloats-nextPos); // update stillNeeded	   ---> should be stillNeeded = 0;
			}
		}
		//System.out.format("consumeSamples return sampleAmount = %s, bufferListLen = %s, headItemLen = %s \n", amountSamples, this.buffer.size(),this.buffer.peekFirst().length);
	...
	*/
	/*
	public synchronized LinkedList<Sample> removeSamples(int num) throws InterruptedException  {
	...
	*/
	/*
	public synchronized LinkedList<Float> removeSamplesAsFloatList(int num) throws InterruptedException {
	...
	public synchronized float[] removeSamplesAsFloatArray(int num) throws InterruptedException {
		LinkedList<Sample> samples = this.removeSamples(num);
	...
	public synchronized int removeSamplesAsFloatArray(float[] out, int num) throws InterruptedException {
		...
		//System.out.format("repacked finished. pos = %s  ; num = %s",pos,num);
		// note: it is expected that pos = num * 2  after this loop!
		return pos;
	}
	*/
