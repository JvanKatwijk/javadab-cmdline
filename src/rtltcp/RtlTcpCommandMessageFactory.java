...
public class RtlTcpCommandMessageFactory {
	...
	private void autosendIfAttached (byte [] commandMessage) {
	   ...
	   if (this. attachedTcpClient == null) {
	      return;
	   }
...
//	attention ... unclear situation with signed/usigned
//	.. use only 0 and positive low values to avoid problems!
...
//	use only 0 and positive low values to avoid problems!
	...
	public byte [] setCenterFrequency (int frequencyHz) { // 0x01
	   ...
	   byte [] commandMessage = encode (0x1, frequencyHz);
	   this. autosendIfAttached (commandMessage);
	   return commandMessage;
	}

	public byte [] setSampleRate (int samplerate) { // 0x02
	   System. out. format ("set sample rate %s Hz\n", samplerate);
	   byte [] commandMessage = encode (0x2, samplerate);
	   this. autosendIfAttached (commandMessage);
	   return commandMessage;
	}
