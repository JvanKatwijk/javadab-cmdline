...
//	it uses an inputStream it repacks in a dataStream
//	(for simple float reading) and handles this in its own thread
...
public class TcpClient  {
	...
	public TcpClient (String sourcehost,
	                  int sourceport,
	                  SampleFIFOBuffer buffer)  {
...
//	if something fails this loop is able to re-establish
//	the tcp connection
	private void lifecycleLoop () {
	   while (!this. ending) {
	      ...
	      this. closeSocket ();
	      try {
//	against too high retry rates if something fails
//	(e.g. server temp down)
	         Thread. sleep (250);
	      } catch (InterruptedException e) {
	         e. printStackTrace ();
	      }
